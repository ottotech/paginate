...
func Test_InnerJoin_Mysql_Employees_That_Are_Managers(t *testing.T) {
	type Employee struct {
		ID         int       `paginate:"id;col=id"`
		Name       string    `paginate:"col=name"`
		LastName   string    `paginate:"col=last_name"`
	...
	innerClause, err := NewInnerJoinClause("mysql")
	if err != nil {
		t.Fatal(err)
	}
	...
	rows, err := mysqlTestDB.Query(sql, args...)
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
...
func Test_InnerJoin_Mysql_Employees_That_Are_Go_Developers(t *testing.T) {
...
func Test_InnerJoin_Mysql_Employees_That_Are_Python_Developers_With_Pagination(t *testing.T) {
	type Employee struct {
		ID                  int       `paginate:"id;col=id"`
		Name                string    `paginate:"col=name"`
		LastName            string    `paginate:"col=last_name"`
	...
	pag, err := NewPaginator(Employee{}, "mysql", *u, TableName("employees"), opt)
	if err != nil {
		log.Fatalln(err)
	}
